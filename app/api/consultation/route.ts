...
const ODOO_API_TOKEN =
  process.env.ODOO_API_TOKEN?.trim() || "";
const allowedTopics = [
  "Website Development",
...
try {
  ...
  console.error(
    ...
    try {
      if (!ODOO_API_TOKEN) {
        throw new Error("ODOO_API_TOKEN is missing in the server environment.");
      }
      ...
      try {
        ...
        try {
          ...
          console.error("Odoo returned invalid JSON", {
        ...
        console.log("Odoo consultation API response", {
          url: ODOO_API_URL,
          status: response.status,
          success: data?.result?.success ?? false,
          duplicate: data?.result?.duplicate ?? false,
          leadId: data?.result?.lead_id ?? null,
          errorCode: data?.error?.code ?? null,
          errorMessage: data?.error?.message ?? null,
        });

        if (!response.ok || data?.error || !data?.result?.success) {
          throw new Error(
            data?.error?.message ||
              data?.result?.message ||
              `Odoo lead creation failed with status ${response.status}.`
          );
        }

        console.log("Bizzfi consultation lead created successfully in Odoo", {
          leadId: data.result.lead_id ?? null,
          duplicate: data.result.duplicate ?? false,
          topic,
        });
      ...
      console.error("Consultation saved, but Odoo lead creation failed", {
        error: odooError instanceof Error ? odooError.message : String(odooError),
        url: ODOO_API_URL,
        hasToken: Boolean(ODOO_API_TOKEN),
      });
    }
